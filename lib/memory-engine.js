...
module.exports = function (opts) {
  ...
  function create(object, callback) {
    ...
    data.push(_.extend({}, extendedObject))
  ...
  function read(id, callback) {
    ...
    findByQuery(query, {}, function (error, objects) {
      if (objects[0] !== undefined) {
        callback(undefined, _.extend({}, objects[0]))
      } else {
  ...
  function update(object, overwrite, callback) {
    ...
    checkForIdAndData(object, function (error, foundObject) {
      ...
      if (!overwrite) {
        updatedObject = _.extend({}, foundObject, object)
      }
      var query = {}
      query[ options.idProperty ] = id
  ...
  function findByQuery(query, options, callback) {
    ...
    var cursor = Mingo.find(data, query)
    if (options && options.sort) {
      cursor = cursor.sort(options.sort)
    }

    if (callback === undefined) {
      return es.readArray(cursor.all())
    } else {
      callback(null, cursor.all())
    }
  ...
  function find(query, options, callback) {
    ...
    return findByQuery(query, options, callback)
  }
